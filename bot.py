...
# ============ CONFIGURATION ============
# Set allowed server IDs here. Leave empty [] to log ALL servers.
# Example: ALLOWED_SERVERS = ["123456789", "987654321"]
ALLOWED_SERVERS = os.getenv('ALLOWED_SERVERS', '').split(',') if os.getenv('ALLOWED_SERVERS') else []

# Set blocked channel IDs here. Leave empty [] to log ALL channels.
# Example: BLOCKED_CHANNELS = ["111111111", "222222222"]
BLOCKED_CHANNELS = os.getenv('BLOCKED_CHANNELS', '').split(',') if os.getenv('BLOCKED_CHANNELS') else []

# Set to True to ignore bot messages
IGNORE_BOTS = os.getenv('IGNORE_BOTS', 'false').lower() == 'true'
# =======================================
...
class MessageLoggerBot(discord.Client):
    ...
    async def on_ready(self):
        ...
        if ALLOWED_SERVERS:
            logger.info(f'Whitelist enabled. Only logging from {len(ALLOWED_SERVERS)} server(s)')
        else:
            logger.info('Logging from ALL servers (no whitelist)')

        if BLOCKED_CHANNELS:
            logger.info(f'Ignoring {len(BLOCKED_CHANNELS)} blocked channel(s)')
    ...
    async def on_message(self, message):
        ...
        if ALLOWED_SERVERS:
            if message.guild and str(message.guild.id) not in ALLOWED_SERVERS:
                return
    ...
    async def on_message_edit(self, before, after):
        ...
        # Check server whitelist
        if ALLOWED_SERVERS:
            if after.guild and str(after.guild.id) not in ALLOWED_SERVERS:
                return

        # Check blocked channels
        if str(after.channel.id) in BLOCKED_CHANNELS:
            return
